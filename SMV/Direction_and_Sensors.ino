...
void backward() {
  analogWrite(L_ENA, LA_V); analogWrite(L_ENB, LB_V);
  analogWrite(R_ENA, RA_V); analogWrite(R_ENB, RB_V);
...
void turn_left() {
  digitalWrite(LED_L, HIGH); digitalWrite(LED_R, LOW);
  analogWrite(L_ENA, LA_V); analogWrite(L_ENB, LB_V);
  analogWrite(R_ENA, RA_V); analogWrite(R_ENB, RB_V);
  digitalWrite(MR_IN1, HIGH); digitalWrite(MR_IN2, LOW);
  digitalWrite(MR_IN3, HIGH); digitalWrite(MR_IN4, LOW);
...
void turn_right() {
  digitalWrite(LED_R, HIGH); digitalWrite(LED_L, LOW);
  analogWrite(L_ENA, LA_V); analogWrite(L_ENB, LB_V);
  analogWrite(R_ENA, RA_V); analogWrite(R_ENB, RB_V);
  digitalWrite(MR_IN1, LOW); digitalWrite(MR_IN2, HIGH);
  digitalWrite(MR_IN3, LOW); digitalWrite(MR_IN4, HIGH);
...
void stop() {
  digitalWrite(LED_R, LOW); digitalWrite(LED_L, LOW);
  analogWrite(L_ENA, LA_V); analogWrite(L_ENB, LB_V);
  analogWrite(R_ENA, RA_V); analogWrite(R_ENB, RB_V);
  digitalWrite(MR_IN1, LOW); digitalWrite(MR_IN2, LOW);
  digitalWrite(MR_IN3, LOW); digitalWrite(MR_IN4, LOW);
...
void processAGVMovement(String inputValue) {
  switch (inputValue.toInt()) {
    case UP: forward(); break;
    case DOWN: backward(); break;
    case UP_LEFT: slide_left_front(); break;
    case UP_RIGHT: slide_right_front(); break;
    case DOWN_LEFT: slide_left_back(); break;
    case DOWN_RIGHT: slide_right_back(); break;
    case TURN_LEFT: turn_left(); break;
    case TURN_RIGHT: turn_right(); break;
    case ON_BUZZER: beep(1); break;
    case OFF_BUZZER: beep(0); break;
    case STOP: 
      stop(); 
      break;
    default: stop(); break;
...
void lcd_fn() {
  ...
  if (currentMillis - previousMillis1 >= lcd_refresh) {
    ...
    if (lcd_mode == 0) {
      ...
      lcd.backlight();
      lcd.setCursor(0, 0); lcd.print(LCDtext0);
      lcd.setCursor(0, 1); lcd.print(LCDtext1);
    }
  }
}
